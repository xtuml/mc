...
.function SyncServiceDefineDeferred
  ...
  ${te_string.strcpy}( ilbargs->${te_parm.GeneratedName}, ${te_parm.GeneratedName} );
      .else
...
.function CreateSynchronousServiceClassDeclaration
  .param inst_ref s_dom
  .//
  .select any te_file from instances of TE_FILE
  .select many s_syncs related by s_dom->S_SYNC[R23]
  .//
  .for each s_sync in s_syncs
    .select one te_aba related by s_sync->TE_SYNC[R2023]->TE_ABA[R2010]
    .select one te_sync related by s_sync->TE_SYNC[R2023]
...
.function CreateSynchronousServiceClassDefinition
  ...
  .select one te_c related by s_dom->TE_C[R2017]
  .select many te_syncs related by s_dom->S_SYNC[R23]->TE_SYNC[R2023] where ( selected.XlateSemantics )
