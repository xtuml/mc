...
${methods.body}\
	      ...
	      .//.print "RenderObjectRelationships subtype_supertype_rel"
	      .//.print "${methods.body}"
${methods.body}\
          .else
...
.//-- 010:20140318 Modified Start (nomura)
      .//.invoke r = GetAttributeCodeGenType( ref_o_attr )
      .invoke r = GetAttributeDataType( ref_o_attr )
.//-- 010:20140318 Modified End (nomura)
...
.//-- 010:20140225 Add End (nomura)
  ...
  .end if
  .//
.//.print "---- this_o_obj.Obj_ID: ${this_o_obj.Obj_ID}"
.//.print "---- related_o_obj.Obj_ID: ${related_o_obj.Obj_ID}"
.//.print "---- te_relinfo.is_formalizer: ${te_relinfo.is_formalizer}"
  ...
  .else
    ...
    .assign data_fini = ""
    .//
.//.print "---- te_relinfo.multiplicity: ${te_relinfo.multiplicity}"
.//.print "---- storage_needed: ${storage_needed}"
    .if ( te_relinfo.multiplicity == 0 )
      .// relationship data storage for link to ONE
      .if ( not te_relinfo.is_supertype )
        .if ( storage_needed )
...
.//-- 010:20140225 Modified End (nomura)
        .end if
      .else
        .if ( te_relinfo.generate_subtype )
          .// optimized relationship storage for link to a subtype
