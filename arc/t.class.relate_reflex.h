...
${externstatic}void ${relate_method.name}( ${te_class.GeneratedName} *, ${te_class.GeneratedName} *${thismodule} );
.else
/* Note:  R${te_rel.Numb}.'${rel_phrase}' never related (or not needed).  */
.end if
.if ( te_rel.UnlinkNeeded or te_c.OptDisabled )
${externstatic}void ${unrelate_method.name}( ${te_class.GeneratedName} *, ${te_class.GeneratedName} *${thismodule} );
.else
