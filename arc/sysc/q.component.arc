...
.function TE_C_GetOtherWayConnection
  ...
  .else
    ...
    .else
      ...
      .if(not_empty te_ci)
        .select one cl_iir_requirer related by te_ci->CL_IC[R2009]->CL_POR[R4707]->CL_IIR[R4708] where (selected.Ref_Id == c_ir.Id)
        .select one cl_ic_provider related by cl_iir_requirer->CL_IR[R4703]->C_SF[R4706]->CL_IPINS[R4705]->CL_IP[R4705]->CL_IIR[R4703]->CL_POR[R4708]->CL_IC[R4707] where ( ( "${selected->PE_PE[R8001].Package_ID}" == pkg_Id ) or ( "${selected.ParentComp_Id}" == parent_c_c_id ) )
      ...
      .// --- 3- Since you are not a component reference, use c_ir to get the satsifaction
      .else
        .select one cl_ic_provider related by c_ir->C_R[R4009]->C_SF[R4002]->CL_IPINS[R4705]->CL_IP[R4705]->CL_IIR[R4703]->CL_POR[R408]->CL_IC[R4707] where ( ("${selected->PE_PE[R8001].Package_ID}" == pkg_Id) or ("${selected.ParentComp_Id}" == parent_c_c_id ) )
