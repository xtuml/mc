...
.if ( te_tim.keyed_timer_support )
  ...
  return ( rc );
.else
  return cancel( (ETimer_t * const) ee_timer_inst_ref );
.end if
}
...
.if ( te_tim.recurring_timer_support )
  if ( 0 != t->recurrence ) {
    ...
    ((sys_sets*) t->event->thismodule)->${te_string.memmove}( e, t->event, sizeof( ${te_eq.base_event_type} ) );
    t->event = e;
