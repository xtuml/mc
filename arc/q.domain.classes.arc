...
.function CreateDomainInitSegment
  ...
  .if ( not_empty prov_init_sgn )
    ...
    .select any foreign_te_po related by te_c->TE_SF[R2203.'has requirement connected to']->TE_MACT[R2200]->TE_PO[R2006] where ( not selected.Provision )
    .select any foreign_te_c related by te_c->TE_SF[R2203.'has requirement connected to']->TE_C[R2203.'has requirement connected to']
  ${prov_init_sgn.GeneratedName}( "${te_c.Name}_${te_po.Name}", "${foreign_te_c.Name}_${foreign_te_po.Name}" );
  .end if
  ...
  .if ( not_empty req_init_sgn )
    ...
    .select any foreign_te_c related by te_c->TE_SF[R2203.'has provision connected to']->TE_C[R2203.'has provision connected to']
  ${req_init_sgn.GeneratedName}( "${te_c.Name}_${te_po.Name}", "${foreign_te_c.Name}_${foreign_te_po.Name}" );
  .end if
.end function
.//
