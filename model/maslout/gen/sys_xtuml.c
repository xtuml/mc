...
{
  Escher_size_t len = 0;
  i_t i = ESCHER_SYS_MAX_STRING_LEN * 4;
  if ( s != 0 ) {
    while ( ( *s != 0 ) && ( i >= 0 ) ) {
      s++;
      len++;
...
extern Escher_idf masl_instance_dumpers[ masl_MAX_CLASS_NUMBERS ];
extern Escher_idf maslout_instance_dumpers[ maslout_MAX_CLASS_NUMBERS ];
static Escher_idf * instance_dumpers[ SYSTEM_DOMAIN_COUNT ] = {
  maslout_instance_dumpers,
  masl_instance_dumpers,
};
