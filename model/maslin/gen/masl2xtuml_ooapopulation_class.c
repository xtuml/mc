...
masl2xtuml_ooapopulation_op_populate( c_t * p_element, c_t p_value[8][ESCHER_SYS_MAX_STRING_LEN] )
{
  ...
  if ( ( Escher_strcmp( "project", element ) == 0 ) ) {
    ...
    /* IF ( not_empty s_sync ) */
    if ( ( 0 != s_sync ) ) {
      /* ASSIGN s_sync.Descrip = PARAM.value[0] */
      s_sync->Descrip = Escher_strcpy( s_sync->Descrip, p_value[0] );
    }
    /* ASSIGN o_tfr = ooapopulation.current_class_op */
    o_tfr = ooapopulation->current_class_op;
    /* IF ( not_empty o_tfr ) */
    if ( ( 0 != o_tfr ) ) {
      /* ASSIGN o_tfr.Descrip = PARAM.value[0] */
      o_tfr->Descrip = Escher_strcpy( o_tfr->Descrip, p_value[0] );
    }
    ...
    if ( ( 0 != sm_state ) ) {
      ...
      /* IF ( not_empty sm_act ) */
      if ( ( 0 != sm_act ) ) {
        /* ASSIGN sm_act.Descrip = PARAM.value[0] */
...
masl2xtuml_ooapopulation_op_transformTerminator( masl2xtuml_ooapopulation * self, c_t * p_name )
{
  ...
  /* ASSIGN comp_if_name = ( PARAM.name + self.current_component.Name ) */
  comp_if_name = Escher_strcpy( comp_if_name, Escher_stradd( p_name, self->current_component->Name ) );
  /* ASSIGN lib_pkg = self.lib_pkg */
  lib_pkg = self->lib_pkg;
