...
ooaofooa_SP_SE_instanceloader( Escher_iHandle_t instance, const c_t * avlstring[] )
{
  ...
  self->modelRootId = Escher_strcpy( self->modelRootId, avlstring[ 3 ] );
  self->className = Escher_strcpy( self->className, avlstring[ 4 ] );
...
ooaofooa_SP_SE_instancedumper( Escher_iHandle_t instance )
{
  ...
  printf( "INSERT INTO SP_SE VALUES ( %ld,%ld,'%s','%s',%d );\n",
    ...
    ( 0 != self->modelRootId ) ? self->modelRootId : "",
    ( 0 != self->className ) ? self->className : "",
    self->elementId );
}
/*
