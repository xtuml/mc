...
ooaofooa_COMM_LNK_instanceloader( Escher_iHandle_t instance, const c_t * avlstring[] )
{
  ...
  self->Numb = Escher_strcpy( self->Numb, avlstring[ 3 ] );
  {i_t i = avlstring[ 4 + 1 ] - avlstring[ 4 ];
  self->Descrip = ( i > 0 ) ? (c_t *) Escher_malloc( i ) : "";
  while ( --i >= 0 ) { self->Descrip[ i ] = avlstring[ 4 ][ i ]; }
  }
  self->StartText = Escher_strcpy( self->StartText, avlstring[ 5 ] );
  self->EndText = Escher_strcpy( self->EndText, avlstring[ 6 ] );
...
ooaofooa_COMM_LNK_instancedumper( Escher_iHandle_t instance )
{
  ...
  printf( "INSERT INTO COMM_LNK VALUES ( %ld,%ld,'%s','%s','%s','%s',%d,%d,%d,%ld,%ld );\n",
    ...
    ( 0 != self->Numb ) ? self->Numb : "",
    ( 0 != self->Descrip ) ? self->Descrip : "",
    ( 0 != self->StartText ) ? self->StartText : "",
    ( 0 != self->EndText ) ? self->EndText : "",
    self->isFormal,
