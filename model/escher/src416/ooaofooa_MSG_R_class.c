...
ooaofooa_MSG_R_instanceloader( Escher_iHandle_t instance, const c_t * avlstring[] )
{
  ...
  self->ResultTarget = Escher_strcpy( self->ResultTarget, avlstring[ 5 ] );
  self->ReturnValue = Escher_strcpy( self->ReturnValue, avlstring[ 6 ] );
  self->SequenceNumb = Escher_strcpy( self->SequenceNumb, avlstring[ 7 ] );
...
ooaofooa_MSG_R_instancedumper( Escher_iHandle_t instance )
{
  ...
  printf( "INSERT INTO MSG_R VALUES ( %ld,'%s','%s','%s','%s','%s','%s' );\n",
    ...
    ( 0 != self->ResultTarget ) ? self->ResultTarget : "",
    ( 0 != self->ReturnValue ) ? self->ReturnValue : "",
