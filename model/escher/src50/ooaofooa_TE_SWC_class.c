...
ooaofooa_TE_SWC_instanceloader( Escher_iHandle_t instance, const c_t * avlstring[] )
{
  ...
  self->Obj_Kl = Escher_strcpy( self->Obj_Kl, avlstring[ 1 ] );
  self->Where_Spec = Escher_strcpy( self->Where_Spec, avlstring[ 2 ] );
  self->Key = Escher_strcpy( self->Key, avlstring[ 3 ] );
  self->Ret_Val = ( '0' != *avlstring[ 4 ] );
  self->Built_In = ( '0' != *avlstring[ 5 ] );
  self->Oid_ID = Escher_atoi( avlstring[ 6 ] );
  self->GeneratedName = Escher_strcpy( self->GeneratedName, avlstring[ 7 ] );
...
ooaofooa_TE_SWC_R2001_Link( ooaofooa_TE_CLASS * part, ooaofooa_TE_SWC * form )
{
  ...
  form->GeneratedName = Escher_strcpy( form->GeneratedName, part->GeneratedName );
...
ooaofooa_TE_SWC_instancedumper( Escher_iHandle_t instance )
{
  ...
  printf( "INSERT INTO TE_SWC VALUES ( '%s','%s','%s',%d,%d,%d,'%s' );\n",
    ...
    ( 0 != self->Where_Spec ) ? self->Where_Spec : "",
    ( 0 != self->Key ) ? self->Key : "",
    self->Ret_Val,
    self->Built_In,
    self->Oid_ID,
