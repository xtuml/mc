...
ooaofooa_TM_SYSTAG_instanceloader( Escher_iHandle_t instance, const c_t * avlstring[] )
{
  ...
  self->SystemCPortsType = Escher_strcpy( self->SystemCPortsType, avlstring[ 15 ] );
  self->AllPortsPoly = ( '0' != *avlstring[ 16 ] );
  self->StructuredMessaging = ( '0' != *avlstring[ 17 ] );
  self->NetworkSockets = ( '0' != *avlstring[ 18 ] );
...
{
  ooaofooa_TM_SYSTAG * self = (ooaofooa_TM_SYSTAG *) instance;
  printf( "INSERT INTO TM_SYSTAG VALUES ( %d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,'%s',%d,%d,%d );\n",
    self->MaxStringLen,
    self->MaxRelExtent,
