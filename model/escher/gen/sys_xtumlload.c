...
)
{
  ...
  if ( n < 0 + ooaofooa_MAX_CLASS_NUMBERS ) {
  ...
  while ( ! done ) {
    ...
    if ( container() ) {
      if ( 0 != wordindex ) {
        ...
        if ( wordindex > 2 ) {
          ooaofooa_load_activity_code_block( (c_t *) word[ 2 ], (c_t *) word[ 0 ], (c_t *) word[ 1 ] );
        } else {
          ooaofooa_load_activity_code_block( (c_t *) word[ 1 ], (c_t *) word[ 0 ], "" );
        }
      }
    } else {
