...
ooaofooa_TE_SM_instanceloader( Escher_iHandle_t instance, const c_t * avlstring[] )
{
  ...
  self->initial_state = Escher_strcpy( self->initial_state, avlstring[ 3 ] );
  self->SEMname = Escher_strcpy( self->SEMname, avlstring[ 4 ] );
  self->action_array = Escher_strcpy( self->action_array, avlstring[ 5 ] );
  self->action_type = Escher_strcpy( self->action_type, avlstring[ 6 ] );
  self->events_union = Escher_strcpy( self->events_union, avlstring[ 7 ] );
  self->txn_action_array = Escher_strcpy( self->txn_action_array, avlstring[ 8 ] );
  self->txn_action_count = Escher_atoi( avlstring[ 9 ] );
  self->state_names_array = Escher_strcpy( self->state_names_array, avlstring[ 10 ] );
  ...
  self->te_classGeneratedName = Escher_strcpy( self->te_classGeneratedName, avlstring[ 14 ] );
...
ooaofooa_TE_SM_instancedumper( Escher_iHandle_t instance )
{
  ...
  printf( "INSERT INTO TE_SM VALUES ( %ld,%d,'%s','%s','%s','%s','%s','%s',%d,'%s',%d,%d,%ld,'%s' );\n",
    ...
    self->complete,
    ( 0 != self->initial_state ) ? self->initial_state : "",
    ( 0 != self->SEMname ) ? self->SEMname : "",
    ( 0 != self->action_array ) ? self->action_array : "",
    ( 0 != self->action_type ) ? self->action_type : "",
    ( 0 != self->events_union ) ? self->events_union : "",
    ( 0 != self->txn_action_array ) ? self->txn_action_array : "",
    self->txn_action_count,
    ( 0 != self->state_names_array ) ? self->state_names_array : "",
    self->num_states,
    self->num_events,
