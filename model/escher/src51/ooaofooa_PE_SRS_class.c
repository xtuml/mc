 ...
 */
ooaofooa_PE_SRS *
ooaofooa_PE_SRS_AnyWhere1( Escher_UniqueID_t w_Package_ID, c_t * w_Name, sys_ElementTypeConstants_t w_Type )
{
  ooaofooa_PE_SRS * w; 
  ...
  while ( (w = (ooaofooa_PE_SRS *) Escher_IteratorNext( &iter_PE_SRS )) != 0 ) {
    if ( w->Package_ID == w_Package_ID && w->Name == w_Name && w->Type == w_Type ) {
      return w;
    }
  }
  return 0;
