...
ooaofooa_CL_IIR_instanceloader( Escher_iHandle_t instance, const c_t * avlstring[] )
{
  ...
  self->CL_POR_Id = (Escher_iHandle_t) Escher_atoi( avlstring[ 3 ] );
...
void ooaofooa_CL_IIR_batch_relate( Escher_iHandle_t instance )
{
  ...
  {
  ooaofooa_CL_POR * ooaofooa_CL_PORrelated_instance1 = (ooaofooa_CL_POR *) Escher_instance_cache[ (intptr_t) ooaofooa_CL_IIR_instance->CL_POR_Id ];
  if ( ooaofooa_CL_PORrelated_instance1 ) {
    ooaofooa_CL_IIR_R4708_Link_exposes( ooaofooa_CL_PORrelated_instance1, ooaofooa_CL_IIR_instance );
  }
  }
}
...
/*
 * RELATE CL_POR TO CL_IIR ACROSS R4708
 */
void
ooaofooa_CL_IIR_R4708_Link_exposes( ooaofooa_CL_POR * part, ooaofooa_CL_IIR * form )
{
  /* Use TagEmptyHandleDetectionOn() to detect empty handle references.  */
  form->CL_POR_Id = part->Id;
  form->CL_POR_R4708_originates_from = part;
  Escher_SetInsertElement( &part->CL_IIR_R4708_exposes, (Escher_ObjectSet_s *) form );
}

/*
 * UNRELATE CL_POR FROM CL_IIR ACROSS R4708
 */
void
ooaofooa_CL_IIR_R4708_Unlink_exposes( ooaofooa_CL_POR * part, ooaofooa_CL_IIR * form )
{
  /* Use TagEmptyHandleDetectionOn() to detect empty handle references.  */
  form->CL_POR_Id = 0;
  form->CL_POR_R4708_originates_from = 0;
  Escher_SetRemoveElement( &part->CL_IIR_R4708_exposes, (Escher_ObjectSet_s *) form );
}
...
ooaofooa_CL_IIR_instancedumper( Escher_iHandle_t instance )
{
  ...
  printf( "INSERT INTO CL_IIR VALUES ( %ld,%ld,%ld,%ld,'%s','%s' );\n",
    ...
    ((long)self->CL_POR_Id & ESCHER_IDDUMP_MASK),
