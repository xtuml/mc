...
ooaofooa_TE_EVENT_PARAMETERS_instanceloader( Escher_iHandle_t instance, const c_t * avlstring[] )
{
  ...
  self->evt_msg_var = Escher_strcpy( self->evt_msg_var, avlstring[ 1 ] );
  self->parameter = Escher_strcpy( self->parameter, avlstring[ 2 ] );
  self->value = Escher_strcpy( self->value, avlstring[ 3 ] );
  self->value_type = Escher_strcpy( self->value_type, avlstring[ 4 ] );
...
ooaofooa_TE_EVENT_PARAMETERS_instancedumper( Escher_iHandle_t instance )
{
  ...
  printf( "INSERT INTO TE_EVENT_PARAMETERS VALUES ( '%s','%s','%s','%s',%ld );\n",
    ( 0 != self->evt_msg_var ) ? self->evt_msg_var : "",
    ( 0 != self->parameter ) ? self->parameter : "",
    ( 0 != self->value ) ? self->value : "",
    ( 0 != self->value_type ) ? self->value_type : "",
