...
ooaofooa_TE_OIR_instanceloader( Escher_iHandle_t instance, const c_t * avlstring[] )
{
  ...
  Escher_strcpy( self->data_member, avlstring[ 1 ] );
  Escher_strcpy( self->assoc_type, avlstring[ 2 ] );
  Escher_strcpy( self->object_id, avlstring[ 3 ] );
  Escher_strcpy( self->rel_phrase, avlstring[ 4 ] );
