...
ooaofooa_DOC_PAR_instanceloader( Escher_iHandle_t instance, const c_t * avlstring[] )
{
  ...
  Escher_strcpy( self->text, avlstring[ 3 ] );
  Escher_strcpy( self->ptext, avlstring[ 4 ] );
 ...
 */
ooaofooa_DOC_PAR *
ooaofooa_DOC_PAR_op_populate( ooaofooa_DOC_SEC * p_doc_sec, ooaofooa_DOC_PAR * p_predecessor, c_t p_text[ESCHER_SYS_MAX_STRING_LEN] )
{
  ...
  Escher_strcpy( doc_par->text, p_text );
  /* ASSIGN doc_par.ptext = PARAM.text */
  doc_par->ptext = p_text;
