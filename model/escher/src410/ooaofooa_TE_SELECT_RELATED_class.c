...
ooaofooa_TE_SELECT_RELATED_instanceloader( Escher_iHandle_t instance, const c_t * avlstring[] )
{
  ...
  Escher_strcpy( self->where_clause, avlstring[ 8 ] );
  Escher_strcpy( self->where_clause_OAL, avlstring[ 9 ] );
  Escher_strcpy( self->multiplicity, avlstring[ 10 ] );
  self->is_implicit = ( '0' != *avlstring[ 11 ] );
  Escher_strcpy( self->result_var, avlstring[ 12 ] );
  Escher_strcpy( self->result_var_OAL, avlstring[ 13 ] );
  self->start_many = ( '0' != *avlstring[ 14 ] );
  Escher_strcpy( self->start_var, avlstring[ 15 ] );
  Escher_strcpy( self->start_var_OAL, avlstring[ 16 ] );
  Escher_strcpy( self->te_classGeneratedName, avlstring[ 17 ] );
