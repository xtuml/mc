...
ooaofooa_TE_CIA_instanceloader( Escher_iHandle_t instance, const c_t * avlstring[] )
{
  ...
  Escher_strcpy( self->class_info_name, avlstring[ 1 ] );
  Escher_strcpy( self->class_info_type, avlstring[ 2 ] );
  Escher_strcpy( self->active_count, avlstring[ 3 ] );
  Escher_strcpy( self->class_count, avlstring[ 4 ] );
  Escher_strcpy( self->count_type, avlstring[ 5 ] );
...
ooaofooa_TE_CIA_instancedumper( Escher_iHandle_t instance )
{
  ...
  printf( "INSERT INTO TE_CIA VALUES ( '%s','%s','%s','%s','%s' );\n",
    self->class_info_name,
    self->class_info_type,
    self->active_count,
    self->class_count,
    self->count_type );
}
