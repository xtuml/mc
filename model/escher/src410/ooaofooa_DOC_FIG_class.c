...
ooaofooa_DOC_FIG_instanceloader( Escher_iHandle_t instance, const c_t * avlstring[] )
{
  ...
  Escher_strcpy( self->ident, avlstring[ 2 ] );
 ...
 */
ooaofooa_DOC_FIG *
ooaofooa_DOC_FIG_op_populate( ooaofooa_DOC_PAR * p_doc_par, c_t p_image_target[ESCHER_SYS_MAX_STRING_LEN], c_t p_title[ESCHER_SYS_MAX_STRING_LEN] )
{
  ...
  Escher_strcpy( doc_fig->title, p_title );
  /* ASSIGN doc_fig.ident = T::r(s:PARAM.image_target) */
  Escher_strcpy( doc_fig->ident, T_r( p_image_target ) );
  ...
  Escher_strcpy( doc_img->target, Escher_stradd( p_image_target, doc_doc->model_image_ext ) );
  /* RETURN doc_fig */
  return doc_fig;
}
