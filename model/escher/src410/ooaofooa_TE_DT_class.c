...
ooaofooa_TE_DT_instanceloader( Escher_iHandle_t instance, const c_t * avlstring[] )
{
  ...
  Escher_strcpy( self->ExtName, avlstring[ 5 ] );
  Escher_strcpy( self->Include_File, avlstring[ 6 ] );
  Escher_strcpy( self->Initial_Value, avlstring[ 7 ] );
  Escher_strcpy( self->Value, avlstring[ 8 ] );
  self->Is_Enum = ( '0' != *avlstring[ 9 ] );
  Escher_strcpy( self->Owning_Dom_Name, avlstring[ 10 ] );
  Escher_strcpy( self->string_format, avlstring[ 11 ] );
