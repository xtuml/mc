		...
		try {
			if (ctx.lhs != null) {
				if (ctx.WITH() != null) {
					...
					Object expression = loader.call_function("OrderingExpression_initialize", visit(ctx.lhs),
							ctx.REVERSE_ORDERED_BY() != null);
					int index = 1;
					for (Object[] sortComponent : ((Stream<Object[]>) visit(ctx.sortOrder()))
							.collect(Collectors.toList())) {
						loader.call_function("OrderingExpression_sort", expression, sortComponent[1], sortComponent[0],
								index);
						index++;
					}
					return expression;
