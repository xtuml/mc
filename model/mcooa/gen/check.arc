...
.end function
.//
.function simple_participation .// string
  .param string From
  .param string To
  .param integer Numb
  ...
  .param string phrase
.if ( "" != phrase )
  select ${multiplicity} $l{To} related by $l{From}->${To}[R$t{Numb}.'${phrase}'];
.else
  select ${multiplicity} $l{To} related by $l{From}->${To}[R$t{Numb}];
.end if
  if ( empty $l{To} )
    LOG::LogInfo(message: "instance not found ${From}->${To}[R$t{Numb}.'${phrase}']");
    break;
  end if;
.end function
.//
...
.function participant_participation .// string
  ...
  .for each r_part in r_parts
    ...
    .if ( r.result )
      ...
      .if ( not_empty r_form )
        .select one target_o_obj related by r_simp->R_FORM[R208]->R_RGO[R205]->R_OIR[R203]->O_OBJ[R201]
        .if ( not r_form.Cond )
          .assign cardinality = "one"
          .if ( r_form.Mult )
            .assign cardinality = "any"
          .end if
          .assign phrase = ""
          .if ( r_part.Obj_ID == r_form.Obj_ID )
            .assign phrase = r_form.Txt_Phrs
          .end if
          .invoke r = simple_participation( o_obj.Key_Lett, target_o_obj.Key_Lett, r_rel.Numb, cardinality, phrase )
...
.function formalizer_participation .// string
  ...
  .for each r_form in r_forms
    ...
    .if ( r.result )
      ...
      .select one target_o_obj related by r_simp->R_PART[R207]->R_RTO[R204]->R_OIR[R203]->O_OBJ[R201]
      .if ( not r_part.Cond )
        .assign cardinality = "one"
        .if ( r_part.Mult )
          .assign cardinality = "any"
