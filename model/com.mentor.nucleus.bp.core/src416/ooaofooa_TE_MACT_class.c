...
ooaofooa_TE_MACT_instanceloader( Escher_iHandle_t instance, const c_t * avlstring[] )
{
  ...
  self->GeneratedName = Escher_strcpy( self->GeneratedName, avlstring[ 5 ] );
  self->ComponentName = Escher_strcpy( self->ComponentName, avlstring[ 6 ] );
  self->OALParamBuffer = Escher_strcpy( self->OALParamBuffer, avlstring[ 7 ] );
  self->PortName = Escher_strcpy( self->PortName, avlstring[ 8 ] );
  self->InterfaceName = Escher_strcpy( self->InterfaceName, avlstring[ 9 ] );
  self->MessageName = Escher_strcpy( self->MessageName, avlstring[ 10 ] );
  self->Direction = Escher_atoi( avlstring[ 11 ] );
  self->Provision = ( '0' != *avlstring[ 12 ] );
  self->subtypeKL = Escher_strcpy( self->subtypeKL, avlstring[ 13 ] );
...
ooaofooa_TE_MACT_instancedumper( Escher_iHandle_t instance )
{
  ...
  printf( "INSERT INTO TE_MACT VALUES ( %ld,%ld,'%s','%s','%s','%s','%s','%s','%s','%s',%d,%d,'%s',%d,%d,%d,%ld,%ld,%ld,%ld,%ld,%ld,%ld,%ld );\n",
    ...
    ( 0 != self->ComponentName ) ? self->ComponentName : "",
    ( 0 != self->OALParamBuffer ) ? self->OALParamBuffer : "",
    ( 0 != self->PortName ) ? self->PortName : "",
    ( 0 != self->InterfaceName ) ? self->InterfaceName : "",
    ( 0 != self->MessageName ) ? self->MessageName : "",
    self->Direction,
    self->Provision,
