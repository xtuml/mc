...
ooaofooa_TE_SMT_instanceloader( Escher_iHandle_t instance, const c_t * avlstring[] )
{
  ...
  self->OAL = Escher_strcpy( self->OAL, avlstring[ 2 ] );
  self->buffer = Escher_strcpy( self->buffer, avlstring[ 3 ] );
  self->buffer2 = Escher_strcpy( self->buffer2, avlstring[ 4 ] );
  self->trace = Escher_strcpy( self->trace, avlstring[ 5 ] );
  ...
  self->subtypeKL = Escher_strcpy( self->subtypeKL, avlstring[ 8 ] );
...
void ooaofooa_TE_SMT_batch_relate( Escher_iHandle_t instance )
{
  ...
  if ( ooaofooa_TE_SMTrelated_instance1 ) {
    ooaofooa_TE_SMT_R2012_Link_succeeds( ooaofooa_TE_SMTrelated_instance1, ooaofooa_TE_SMT_instance );
  }
  }
  {
