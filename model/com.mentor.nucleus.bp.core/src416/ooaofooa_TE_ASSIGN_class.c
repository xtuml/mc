...
ooaofooa_TE_ASSIGN_instanceloader( Escher_iHandle_t instance, const c_t * avlstring[] )
{
  ...
  self->array_spec = Escher_strcpy( self->array_spec, avlstring[ 3 ] );
  self->left_declaration = Escher_strcpy( self->left_declaration, avlstring[ 4 ] );
  self->Core_Typ = Escher_atoi( avlstring[ 5 ] );
  self->lval = Escher_strcpy( self->lval, avlstring[ 6 ] );
  self->rval = Escher_strcpy( self->rval, avlstring[ 7 ] );
...
ooaofooa_TE_ASSIGN_instancedumper( Escher_iHandle_t instance )
{
  ...
  printf( "INSERT INTO TE_ASSIGN VALUES ( %d,%d,'%s','%s',%d,'%s','%s',%ld,%d );\n",
    ...
    ( 0 != self->left_declaration ) ? self->left_declaration : "",
    self->Core_Typ,
    ( 0 != self->lval ) ? self->lval : "",
    ( 0 != self->rval ) ? self->rval : "",
