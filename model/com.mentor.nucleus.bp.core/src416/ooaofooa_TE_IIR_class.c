...
ooaofooa_TE_IIR_instanceloader( Escher_iHandle_t instance, const c_t * avlstring[] )
{
  ...
  self->component_name = Escher_strcpy( self->component_name, avlstring[ 2 ] );
  self->port_name = Escher_strcpy( self->port_name, avlstring[ 3 ] );
  self->interface_name = Escher_strcpy( self->interface_name, avlstring[ 4 ] );
  self->polymorphic = ( '0' != *avlstring[ 5 ] );
  self->template_parameter_values = Escher_strcpy( self->template_parameter_values, avlstring[ 6 ] );
  self->C2C = Escher_strcpy( self->C2C, avlstring[ 7 ] );
...
ooaofooa_TE_IIR_instancedumper( Escher_iHandle_t instance )
{
  ...
  printf( "INSERT INTO TE_IIR VALUES ( %ld,'%s','%s','%s',%d,'%s','%s',%ld,%ld,%ld,%ld );\n",
    ...
    ( 0 != self->component_name ) ? self->component_name : "",
    ( 0 != self->port_name ) ? self->port_name : "",
    ( 0 != self->interface_name ) ? self->interface_name : "",
    self->polymorphic,
    ( 0 != self->template_parameter_values ) ? self->template_parameter_values : "",
    ( 0 != self->C2C ) ? self->C2C : "",
