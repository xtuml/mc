...
ooaofooa_TE_CLASS_instanceloader( Escher_iHandle_t instance, const c_t * avlstring[] )
{
  ...
  Escher_strcpy( self->scope, avlstring[ 5 ] );
  Escher_strcpy( self->GeneratedName, avlstring[ 6 ] );
  Escher_strcpy( self->CBGeneratedName, avlstring[ 7 ] );
  ...
  Escher_strcpy( self->class_file, avlstring[ 21 ] );
  Escher_strcpy( self->system_class_number, avlstring[ 22 ] );
  Escher_strcpy( self->CBsystem_class_number, avlstring[ 23 ] );
  Escher_strcpy( self->persist_link, avlstring[ 24 ] );
  Escher_strcpy( self->dispatcher, avlstring[ 25 ] );
  Escher_strcpy( self->CBdispatcher, avlstring[ 26 ] );
  Escher_strcpy( self->attribute_format, avlstring[ 27 ] );
  Escher_strcpy( self->attribute_dump, avlstring[ 28 ] );
...
void ooaofooa_TE_CLASS_batch_relate( Escher_iHandle_t instance )
{
  ...
  if ( ooaofooa_TE_CLASSrelated_instance1 ) {
    ooaofooa_TE_CLASS_R2092_Link_precedes( ooaofooa_TE_CLASSrelated_instance1, ooaofooa_TE_CLASS_instance );
  }
  }
}
 ...
 */
ooaofooa_TE_CLASS *
ooaofooa_TE_CLASS_AnyWhere1( c_t w_GeneratedName[ESCHER_SYS_MAX_STRING_LEN] )
{
  ooaofooa_TE_CLASS * w; 
  ...
  while ( (w = (ooaofooa_TE_CLASS *) Escher_IteratorNext( &iter_TE_CLASS )) != 0 ) {
    if ( !Escher_strcmp(w->GeneratedName, w_GeneratedName) ) {
      return w;
    }
  }
  return 0;
...
ooaofooa_TE_CLASS_instancedumper( Escher_iHandle_t instance )
{
  ...
  printf( "INSERT INTO TE_CLASS VALUES ( %ld,'%s',%d,'%s','%s','%s','%s',%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,'%s','%s','%s','%s','%s','%s','%s','%s',%ld,%ld,%ld );\n",
    ...
    self->Task,
    self->class_file,
    self->system_class_number,
    self->CBsystem_class_number,
    self->persist_link,
    self->dispatcher,
    self->CBdispatcher,
