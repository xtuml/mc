...
ooaofooa_TE_CREATE_EVENT_instanceloader( Escher_iHandle_t instance, const c_t * avlstring[] )
{
  ...
  Escher_strcpy( self->event_meaning, avlstring[ 4 ] );
  Escher_strcpy( self->oal_var_name, avlstring[ 5 ] );
  Escher_strcpy( self->var_name, avlstring[ 6 ] );
  Escher_strcpy( self->recipient_var_name, avlstring[ 7 ] );
  Escher_strcpy( self->parameters, avlstring[ 8 ] );
