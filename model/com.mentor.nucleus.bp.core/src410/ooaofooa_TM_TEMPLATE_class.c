...
ooaofooa_TM_TEMPLATE_instanceloader( Escher_iHandle_t instance, const c_t * avlstring[] )
{
  ...
  Escher_strcpy( self->tm_cPackage, avlstring[ 2 ] );
  Escher_strcpy( self->tm_cName, avlstring[ 3 ] );
  Escher_strcpy( self->tm_ifPackage, avlstring[ 4 ] );
  Escher_strcpy( self->tm_ifName, avlstring[ 5 ] );
  self->current = ( '0' != *avlstring[ 6 ] );
  Escher_strcpy( self->template_parameters, avlstring[ 7 ] );
  Escher_strcpy( self->template_parameters_default, avlstring[ 8 ] );
  Escher_strcpy( self->template_actual_parameters, avlstring[ 9 ] );
  Escher_strcpy( self->template_default_instantiation, avlstring[ 10 ] );
...
ooaofooa_TM_TEMPLATE_R2800_Link( ooaofooa_TM_C * part, ooaofooa_TM_TEMPLATE * form )
{
  ...
  Escher_strcpy( form->tm_cPackage, part->Package );
  Escher_strcpy( form->tm_cName, part->Name );
...
{
  /* Use TagEmptyHandleDetectionOn() to detect empty handle references.  */
  Escher_strcpy( form->tm_ifPackage, part->Package );
  Escher_strcpy( form->tm_ifName, part->Name );
...
ooaofooa_TM_TEMPLATE_instancedumper( Escher_iHandle_t instance )
{
  ...
  printf( "INSERT INTO TM_TEMPLATE VALUES ( %ld,'%s','%s','%s','%s',%d,'%s','%s','%s','%s' );\n",
    ...
    self->template_parameters_default,
    self->template_actual_parameters,
    self->template_default_instantiation );
}
/*
