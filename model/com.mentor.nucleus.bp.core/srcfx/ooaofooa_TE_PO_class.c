...
ooaofooa_TE_PO_instanceloader( Escher_iHandle_t instance, const c_t * avlstring[] )
{
  ...
  self->InterfaceName = Escher_strcpy( self->InterfaceName, avlstring[ 4 ] );
  self->PackageName = Escher_strcpy( self->PackageName, avlstring[ 5 ] );
  ...
  self->realizeEE_ID = (Escher_iHandle_t) Escher_atoi( avlstring[ 13 ] );
...
ooaofooa_TE_PO_instancedumper( Escher_iHandle_t instance )
{
  ...
  printf( "INSERT INTO TE_PO VALUES ( %ld,'%s','%s','%s','%s',%d,%d,%d,%d,%ld,%ld,%ld,%ld );\n",
    ...
    ((long)self->realizeEE_ID & ESCHER_IDDUMP_MASK) );
}
/*
