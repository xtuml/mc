...
ooaofooa_TE_CLASS_instanceloader( Escher_iHandle_t instance, const c_t * avlstring[] )
{
  ...
  self->InstanceMaxNo = Escher_strcpy( self->InstanceMaxNo, avlstring[ 32 ] );
  self->InstanceMaxNoDefIncludeFile = Escher_strcpy( self->InstanceMaxNoDefIncludeFile, avlstring[ 33 ] );
...
{
  ooaofooa_TE_CLASS * self = (ooaofooa_TE_CLASS *) instance;
  printf( "INSERT INTO TE_CLASS VALUES ( %ld,'%s',%d,'%s','%s','%s','%s',%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,'%s','%s','%s','%s','%s','%s','%s','%s',%ld,%ld,%ld,'%s','%s' );\n",
    ...
    ( 0 != self->InstanceMaxNo ) ? self->InstanceMaxNo : "",
    ( 0 != self->InstanceMaxNoDefIncludeFile ) ? self->InstanceMaxNoDefIncludeFile : "" );
}
/*
