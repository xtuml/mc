...
ooaofooa_TE_ATTR_instanceloader( Escher_iHandle_t instance, const c_t * avlstring[] )
{
  ...
  self->isAutoInc = ( '0' != *avlstring[ 20 ] );
  self->AutoIncDataSize = Escher_atoi( avlstring[ 21 ] );
  self->AutoIncLowLimit = Escher_strcpy( self->AutoIncLowLimit, avlstring[ 22 ] );
  self->AutoIncHighLimit = Escher_strcpy( self->AutoIncHighLimit, avlstring[ 23 ] );
  self->AutoIncDirection = Escher_strcpy( self->AutoIncDirection, avlstring[ 24 ] );
  self->AutoIncUndefValue = Escher_strcpy( self->AutoIncUndefValue, avlstring[ 25 ] );
...
{
  ooaofooa_TE_ATTR * self = (ooaofooa_TE_ATTR *) instance;
  printf( "INSERT INTO TE_ATTR VALUES ( %ld,%d,%d,%d,%d,%d,'%s','%s','%s','%s',%d,%d,%ld,'%s','%s','%s',%ld,%ld,%ld,%d,%d,'%s','%s','%s','%s' );\n",
    ...
    self->isAutoInc,
    self->AutoIncDataSize,
    ( 0 != self->AutoIncLowLimit ) ? self->AutoIncLowLimit : "",
    ( 0 != self->AutoIncHighLimit ) ? self->AutoIncHighLimit : "",
    ( 0 != self->AutoIncDirection ) ? self->AutoIncDirection : "",
    ( 0 != self->AutoIncUndefValue ) ? self->AutoIncUndefValue : "" );
}
/*
