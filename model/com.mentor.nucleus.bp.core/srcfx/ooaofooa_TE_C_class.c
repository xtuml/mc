...
ooaofooa_TE_C_instanceloader( Escher_iHandle_t instance, const c_t * avlstring[] )
{
  ...
  self->MaxInternalEvents = Escher_atoi( avlstring[ 35 ] );
  self->MaxExternalEvents = Escher_atoi( avlstring[ 36 ] );
  self->MaxRelationExtents = Escher_atoi( avlstring[ 37 ] );
  self->MaxSelectionNodeExtents = Escher_atoi( avlstring[ 38 ] );
  self->isUseMutexLock = ( '0' != *avlstring[ 39 ] );
  self->isUseFacadeMaxDef = ( '0' != *avlstring[ 40 ] );
...
{
  ooaofooa_TE_C * self = (ooaofooa_TE_C *) instance;
  printf( "INSERT INTO TE_C VALUES ( %ld,'%s','%s',%d,%d,%d,%d,%d,%d,%d,%d,'%s','%s',%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,'%s','%s','%s',%d,%d,%d,%d,%ld,%ld,%ld,%ld,%d,%d,%d,%d,%d,%d );\n",
    ...
    self->MaxInternalEvents,
    self->MaxExternalEvents,
    self->MaxRelationExtents,
    self->MaxSelectionNodeExtents,
    self->isUseMutexLock,
    self->isUseFacadeMaxDef );
}
/*
