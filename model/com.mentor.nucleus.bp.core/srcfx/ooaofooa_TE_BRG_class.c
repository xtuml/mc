...
ooaofooa_TE_BRG_instanceloader( Escher_iHandle_t instance, const c_t * avlstring[] )
{
  ...
  self->EEname = Escher_strcpy( self->EEname, avlstring[ 1 ] );
  self->EEkeyletters = Escher_strcpy( self->EEkeyletters, avlstring[ 2 ] );
  ...
  self->NotGenerateInvocation = ( '0' != *avlstring[ 8 ] );
  self->RealFuncName = Escher_strcpy( self->RealFuncName, avlstring[ 9 ] );
  self->IsRealFunc = ( '0' != *avlstring[ 10 ] );
...
{
  ooaofooa_TE_BRG * self = (ooaofooa_TE_BRG *) instance;
  printf( "INSERT INTO TE_BRG VALUES ( '%s','%s','%s','%s',%ld,%ld,%ld,%d,'%s',%d );\n",
    ( 0 != self->EEname ) ? self->EEname : "",
    ( 0 != self->EEkeyletters ) ? self->EEkeyletters : "",
    ...
    self->NotGenerateInvocation,
    ( 0 != self->RealFuncName ) ? self->RealFuncName : "",
    self->IsRealFunc );
}
/*
