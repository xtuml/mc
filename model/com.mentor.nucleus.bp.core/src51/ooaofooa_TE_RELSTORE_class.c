...
ooaofooa_TE_RELSTORE_instanceloader( Escher_iHandle_t instance, const c_t * avlstring[] )
{
  ...
  self->data_declare = Escher_strcpy( self->data_declare, avlstring[ 1 ] );
  self->data_init = Escher_strcpy( self->data_init, avlstring[ 2 ] );
  self->link_calls = Escher_strcpy( self->link_calls, avlstring[ 3 ] );
  self->link_index = Escher_atoi( avlstring[ 4 ] );
  self->data_fini = Escher_strcpy( self->data_fini, avlstring[ 5 ] );
  self->self_name = Escher_strcpy( self->self_name, avlstring[ 6 ] );
...
ooaofooa_TE_RELSTORE_instancedumper( Escher_iHandle_t instance )
{
  ...
  printf( "INSERT INTO TE_RELSTORE VALUES ( '%s','%s','%s',%d,'%s','%s' );\n",
    ( 0 != self->data_declare ) ? self->data_declare : "",
    ( 0 != self->data_init ) ? self->data_init : "",
    ( 0 != self->link_calls ) ? self->link_calls : "",
    self->link_index,
    ( 0 != self->data_fini ) ? self->data_fini : "",
    ( 0 != self->self_name ) ? self->self_name : "" );
}
/*
