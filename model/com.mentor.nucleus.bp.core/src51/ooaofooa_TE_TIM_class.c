...
ooaofooa_TE_TIM_instanceloader( Escher_iHandle_t instance, const c_t * avlstring[] )
{
  ...
  self->max_timers = Escher_strcpy( self->max_timers, avlstring[ 1 ] );
  ...
  self->internal_type = Escher_strcpy( self->internal_type, avlstring[ 5 ] );
  self->scope = Escher_strcpy( self->scope, avlstring[ 6 ] );
  self->event_name = Escher_strcpy( self->event_name, avlstring[ 7 ] );
