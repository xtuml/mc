...
ooaofooa_TE_DCI_instancedumper( Escher_iHandle_t instance )
{
  ...
  printf( "INSERT INTO TE_DCI VALUES ( %ld,'%s','%s','%s','%s','%s','%s','%s','%s' );\n",
    ...
    ( 0 != self->class_numbers ) ? self->class_numbers : "",
    ( 0 != self->persist_union ) ? self->persist_union : "",
    ( 0 != self->task_list ) ? self->task_list : "",
    ( 0 != self->task_numbers ) ? self->task_numbers : "",
    ( 0 != self->max ) ? self->max : "",
    ( 0 != self->max_models ) ? self->max_models : "",
    ( 0 != self->init ) ? self->init : "",
    ( 0 != self->array_name ) ? self->array_name : "" );
}
/*
