...
ooaofooa_TE_FOR_instanceloader( Escher_iHandle_t instance, const c_t * avlstring[] )
{
  ...
  self->class_name = Escher_strcpy( self->class_name, avlstring[ 2 ] );
  self->loop_variable = Escher_strcpy( self->loop_variable, avlstring[ 3 ] );
  self->set_variable = Escher_strcpy( self->set_variable, avlstring[ 4 ] );
...
ooaofooa_TE_FOR_instancedumper( Escher_iHandle_t instance )
{
  ...
  printf( "INSERT INTO TE_FOR VALUES ( %d,'%s','%s','%s',%ld );\n",
    ...
    ( 0 != self->loop_variable ) ? self->loop_variable : "",
    ( 0 != self->set_variable ) ? self->set_variable : "",
