...
ooaofooa_TE_SELECT_WHERE_instanceloader( Escher_iHandle_t instance, const c_t * avlstring[] )
{
  ...
  self->oal_var_name = Escher_strcpy( self->oal_var_name, avlstring[ 4 ] );
  ...
  self->selected_var_name = Escher_strcpy( self->selected_var_name, avlstring[ 8 ] );
  self->where_clause = Escher_strcpy( self->where_clause, avlstring[ 9 ] );
...
ooaofooa_TE_SELECT_WHERE_instancedumper( Escher_iHandle_t instance )
{
  ...
  printf( "INSERT INTO TE_SELECT_WHERE VALUES ( %ld,%d,'%s','%s','%s','%s','%s','%s','%s',%d,%d,%ld );\n",
    ...
    ( 0 != self->selected_var_name ) ? self->selected_var_name : "",
