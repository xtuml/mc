-- ============================================================================
-- $RCSfile: xtumlmc_schema_220.sql,v $
--
-- Description:
...
--
-- Notice:
--   (C) Copyright 1998-2009 Mentor Graphics Corporation
--   All rights reserved.
--
...
-- Class:  114.  Referential Attribute Visited Recorder
CREATE TABLE O_RAVR (
 );
...
CREATE ROP REF_ID R3103	FROM 1C BP_EV	(SM_ID, SMevt_ID)
			  TO 1  SM_EVT	(SM_ID, SMevt_ID);
...
CREATE ROP REF_ID R405	FROM MC CA_EESME	(SM_ID, SMevt_ID)
			  TO 1  SM_EVT	(SM_ID, SMevt_ID);
...
CREATE ROP REF_ID R409	FROM MC CA_SMSME	(SM_ID, SMevt_ID)
			  TO 1  SM_EVT	(SM_ID, SMevt_ID);
...
CREATE ROP REF_ID R4706	FROM 1C CL_IR	(Satisfaction_Id)
			  TO 1C C_SF	(Id);
...
CREATE ROP REF_ID R1501	FROM MC CNST_LSC	(DT_ID)
			  TO 1  S_CDT	(DT_ID);
...
CREATE ROP REF_ID R1505	FROM 1C CNST_SYC	(Previous_Const_ID, Previous_DT_ID) PHRASE 'succeeds'
...
CREATE ROP REF_ID R1507	FROM MC CNST_CSP	(Dom_ID)
			  TO 1C S_DOM	(Dom_ID);
...
CREATE ROP REF_ID R13	FROM MC S_EEEDT	(EE_ID, EEedi_ID)
			  TO 1  S_EEEDI	(EE_ID, EEedi_ID);
...
CREATE ROP REF_ID R44	FROM MC S_MBR	(Parent_DT_ID)
			  TO 1  S_SDT	(DT_ID);

CREATE ROP REF_ID R45	FROM MC S_MBR	(DT_ID)
			  TO 1  S_DT	(DT_ID);

CREATE ROP REF_ID R46	FROM 1C S_MBR	(Previous_Member_ID, Parent_DT_ID) PHRASE 'succeeds'
			  TO 1C S_MBR	(Member_ID, Parent_DT_ID) PHRASE 'precedes';
...
CREATE ROP REF_ID R53	FROM MC S_DIM	(Member_ID, DT_ID)
			  TO 1C S_MBR	(Member_ID, Parent_DT_ID);
...
CREATE ROP REF_ID R706	FROM MC E_CSME	(SM_ID, SMevt_ID)
			  TO 1  SM_EVT	(SM_ID, SMevt_ID);

CREATE ROP REF_ID R707	FROM MC E_GSME	(SM_ID, SMevt_ID)
			  TO 1  SM_EVT	(SM_ID, SMevt_ID);
...
CREATE ROP REF_ID R2901	FROM MC I_LNK	(Rel_ID, fromInst_ID)
			  TO 1  I_LIP	(Rel_ID, Inst_ID);

CREATE ROP REF_ID R2902	FROM MC I_LNK	(Rel_ID, toInst_ID)
			  TO 1  I_LIP	(Rel_ID, Inst_ID);

CREATE ROP REF_ID R2903	FROM MC I_LNK	(Rel_ID, assocInst_ID)
			  TO 1C I_LIP	(Rel_ID, Inst_ID);

CREATE ROP REF_ID R2904	FROM MC I_LNK	(Rel_ID)
			  TO 1  R_REL	(Rel_ID);

CREATE ROP REF_ID R2906	FROM MC I_EVI	(SM_ID, SMevt_ID)
			  TO 1  SM_EVT	(SM_ID, SMevt_ID);
...
CREATE ROP REF_ID R2910	FROM MC I_AVL	(Obj_ID, Attr_ID)
			  TO 1  O_ATTR	(Obj_ID, Attr_ID);
...
CREATE ROP REF_ID R2951	FROM MC I_VSF	(Value_ID)
			  TO 1  V_VAL	(Value_ID);
...
CREATE ROP REF_ID R2965	FROM 1C I_STACK	(Wait_Stack_Frame_ID)
			  TO 1C I_STF	(Stack_Frame_ID);
...
CREATE ROP REF_ID R1009	FROM MC MSG_E	(SM_ID, SMevt_ID)
			  TO 1C SM_EVT	(SM_ID, SMevt_ID);
...
CREATE ROP REF_ID R532	FROM MC SM_EVTDI	(SM_ID, SMevt_ID)
			  TO 1  SM_EVT	(SM_ID, SMevt_ID);
...
CREATE ROP REF_ID R110	FROM M  O_RTIDA	(Obj_ID, Oid_ID, Rel_ID, OIR_ID)
			  TO 1  R_RTO	(Obj_ID, Oid_ID, Rel_ID, OIR_ID);

CREATE ROP REF_ID R110	FROM MC O_RTIDA	(Attr_ID, Obj_ID, Oid_ID)
			  TO 1  O_OIDA	(Attr_ID, Obj_ID, Oid_ID);

CREATE ROP REF_ID R111	FROM MC O_REF	(Obj_ID, Rel_ID, OIR_ID)
			  TO 1  R_RGO	(Obj_ID, Rel_ID, OIR_ID);

CREATE ROP REF_ID R111	FROM MC O_REF	(RAttr_ID, Rel_ID, RObj_ID, ROIR_ID, ROid_ID)
			  TO 1  O_RTIDA	(Attr_ID, Rel_ID, Obj_ID, OIR_ID, Oid_ID);
...
CREATE ROP REF_ID R119	FROM MC O_RATTR	(recorderId)
			  TO 1C O_RAVR	(recorderId);

CREATE ROP REF_ID R120	FROM MC S_DIM	(Obj_ID, Attr_ID)
			  TO 1C O_ATTR	(Obj_ID, Attr_ID);
...
CREATE ROP REF_ID R806	FROM MC V_AVL	(Obj_ID, Attr_ID)
			  TO 1  O_ATTR	(Obj_ID, Attr_ID);
...
CREATE ROP REF_ID R812	FROM MC V_SLR	(Obj_ID, Attr_ID)
			  TO 1C O_ATTR	(Obj_ID, Attr_ID);
...
CREATE ROP REF_ID R836	FROM MC V_MVL	(Member_ID, DT_ID)
			  TO 1  S_MBR	(Member_ID, Parent_DT_ID);
...
-- Class:  2010.  Instance Reference Variable
CREATE TABLE TE_OIV (
	OivID	INTEGER,
	Name	STRING,
	VarName	STRING,
...
-- Class:  2011.  Instance Reference
CREATE TABLE TE_OIR (
	AbaID	UNIQUE_ID,
	Number	INTEGER,
	OivID	INTEGER );

-- Class:  2012.  Extended Domain
CREATE TABLE TE_DOM (
	ID	UNIQUE_ID,
...
-- Class:  2074.  OAL else
-- ERROR:  Class OAL else (TE_ELSE) has no attributes.
...
-- Class:  2086.  OAL select_related_where
CREATE TABLE TE_SELECT_RELATED_WHERE (
	o_obj	UNIQUE_ID,
	is_implicit	BOOLEAN,
	class_name	STRING,
	oal_var_name	STRING,
...
-- ERROR:  Class OAL control (TE_CONTROL) has no attributes.

-- Class:  2099.  OAL break
-- ERROR:  Class OAL break (TE_BREAK) has no attributes.

-- Class:  2100.  OAL continue
-- ERROR:  Class OAL continue (TE_CONTINUE) has no attributes.
...
CREATE ROP REF_ID R2001	FROM MC TE_SWC	(Obj_ID)
			  TO 1C O_OBJ	(Obj_ID);
...
CREATE ROP REF_ID R2012	FROM 1C TE_BLOCK	(AbaID, PreviousNumber) PHRASE 'succeeds'
			  TO 1C TE_BLOCK	(AbaID, Number) PHRASE 'precedes';
...
CREATE ROP REF_ID R2017	FROM 1C TE_DOM	(Dom_ID)
			  TO 1  S_DOM	(Dom_ID);
...
CREATE ROP REF_ID R2032	FROM 1C TE_WHERE	(Obj_ID, Oid_ID)
			  TO 1  O_ID	(Obj_ID, Oid_ID);

CREATE ROP REF_ID R2033	FROM 1C TE_ATTR	(Obj_ID, Attr_ID)
			  TO 1  O_ATTR	(Obj_ID, Attr_ID);

CREATE ROP REF_ID R2034	FROM 1C TE_REL	(Rel_ID)
			  TO 1  R_REL	(Rel_ID);

CREATE ROP REF_ID R2035	FROM 1C TE_NAV	(Obj_ID, OIR_ID, Rel_ID)
			  TO 1  R_OIR	(Obj_ID, OIR_ID, Rel_ID);

CREATE ROP REF_ID R2036	FROM 1C TE_EVT	(SM_ID, SMevt_ID)
			  TO 1  SM_EVT	(SM_ID, SMevt_ID);
...
CREATE ROP REF_ID R2049	FROM 1C TE_PARM	(te_dtID)
			  TO 1  TE_DT	(ID);
...
CREATE ROP REF_ID R2054	FROM 1C TE_C	(Id)
			  TO 1  C_C	(Id);
...
CREATE ROP REF_ID R2064	FROM MC TE_CLASS	(te_domID)
			  TO 1  TE_DOM	(ID);

CREATE ROP REF_ID R2065	FROM MC TE_DOM	(SystemID)
...
-- Note:  R2069 is unformalized from subtype TE_ASSIGN to supertype TE_SMT

-- Note:  R2069 is unformalized from subtype TE_FOR to supertype TE_SMT

-- Note:  R2069 is unformalized from subtype TE_ELSE to supertype TE_SMT

-- Note:  R2069 is unformalized from subtype TE_DELETE_INSTANCE to supertype TE_SMT

-- Note:  R2069 is unformalized from subtype TE_IF to supertype TE_SMT

-- Note:  R2069 is unformalized from subtype TE_WHILE to supertype TE_SMT

-- Note:  R2069 is unformalized from subtype TE_CREATE_INSTANCE to supertype TE_SMT

-- Note:  R2069 is unformalized from subtype TE_ELIF to supertype TE_SMT

-- Note:  R2069 is unformalized from subtype TE_CREATE_EVENT to supertype TE_SMT

-- Note:  R2069 is unformalized from subtype TE_UNRELATE_USING to supertype TE_SMT

-- Note:  R2069 is unformalized from subtype TE_UNRELATE to supertype TE_SMT

-- Note:  R2069 is unformalized from subtype TE_RELATE to supertype TE_SMT

-- Note:  R2069 is unformalized from subtype TE_SELECT to supertype TE_SMT

-- Note:  R2069 is unformalized from subtype TE_RELATE_USING to supertype TE_SMT

-- Note:  R2069 is unformalized from subtype TE_SELECT_RELATED to supertype TE_SMT

-- Note:  R2069 is unformalized from subtype TE_SELECT_WHERE to supertype TE_SMT

-- Note:  R2069 is unformalized from subtype TE_SELECT_RELATED_WHERE to supertype TE_SMT

-- Note:  R2069 is unformalized from subtype TE_GENERATE_PRECREATED_EVENT to supertype TE_SMT

-- Note:  R2069 is unformalized from subtype TE_EVENT_PARAMETERS to supertype TE_SMT

-- Note:  R2069 is unformalized from subtype TE_GENERATE to supertype TE_SMT

-- Note:  R2069 is unformalized from subtype TE_GENERATE_CREATOR_EVENT to supertype TE_SMT

-- Note:  R2069 is unformalized from subtype TE_GENERATE_TO_CLASS to supertype TE_SMT

-- Note:  R2069 is unformalized from subtype TE_SGN to supertype TE_SMT

-- Note:  R2069 is unformalized from subtype TE_IOP to supertype TE_SMT

-- Note:  R2069 is unformalized from subtype TE_OPERATION to supertype TE_SMT

-- Note:  R2069 is unformalized from subtype TE_BRIDGE to supertype TE_SMT

-- Note:  R2069 is unformalized from subtype TE_FUNCTION to supertype TE_SMT

-- Note:  R2069 is unformalized from subtype TE_RETURN to supertype TE_SMT

-- Note:  R2069 is unformalized from subtype TE_CONTROL to supertype TE_SMT

-- Note:  R2069 is unformalized from subtype TE_BREAK to supertype TE_SMT

-- Note:  R2069 is unformalized from subtype TE_CONTINUE to supertype TE_SMT
