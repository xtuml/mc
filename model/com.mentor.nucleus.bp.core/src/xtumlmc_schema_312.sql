-- ============================================================================
-- $RCSfile: xtumlmc_schema_312.sql,v $
--
-- Description:
...
CREATE ROP REF_ID R2966	FROM MC I_STF	(Q_Stack_ID)
			  TO 1C I_STACK	(Stack_ID);
...
CREATE ROP REF_ID R8001	FROM 1C C_IR	(Id)
			  TO 1  PE_PE	(Element_ID);
...
CREATE ROP REF_ID R8001	FROM 1C SQ_TM	(Mark_ID)
			  TO 1  PE_PE	(Element_ID);

CREATE ROP REF_ID R8001	FROM 1C SQ_TS	(Span_ID)
			  TO 1  PE_PE	(Element_ID);
...
CREATE ROP REF_ID R2069	FROM MC TE_VAL	(te_dimID)
			  TO 1C TE_DIM	(te_dimID);
...
CREATE ROP REF_ID R2081	FROM 1C TE_IIR	(provider_te_iirID) PHRASE 'requires or delegates'
