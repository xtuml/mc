...
CREATE ROP REF_ID R2035 FROM 1C TE_OIR (OIR_ID, Rel_ID, Obj_ID) TO 1 R_OIR (OIR_ID, Rel_ID, Obj_ID);
...
CREATE ROP REF_ID R2802 FROM 1C TM_TEMPLATE (tm_ifName, tm_ifPackage) TO 1C TM_IF (Name, Package);
...
CREATE ROP REF_ID R8006 FROM MC PE_VIS (Package_ID, Type, Name) TO 1 PE_SRS (Package_ID, Type, Name);
CREATE ROP REF_ID R8007 FROM MC PE_CRS (Id) TO 1 C_C (Id);
CREATE ROP REF_ID R8008 FROM MC PE_CVS (Type, Name, Id) TO 1 PE_CRS (Type, Name, Id);
...
CREATE ROP REF_ID R2200 FROM 1C TE_SF (satisfaction_Id) TO MC TE_MACT (satisfaction_Id);
CREATE ROP REF_ID R2201 FROM 1 C_SF (Id) TO 1C TE_SF (satisfaction_Id);
