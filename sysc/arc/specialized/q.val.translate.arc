...
.function val_message_value
  ...
  .select any te_c related by te_mact->TE_C[R2002]
  .assign comp_inst_name = ""
  .if ( te_sys.SystemCPortsType == "TLM" )
    .assign comp_inst_name = "i${te_c.Name}.m_${te_c.Name}"
  .else
    .assign comp_inst_name = "i${te_c.Name}"
  .end if
  .assign name = ( te_mact.PortName + "->" ) + te_mact.GeneratedName
...
.function val_transform_value
  ...
  .if ( te_tfr.Instance_Based == 1 )
    ...
    .end if
    .if ( not_empty v_var )
      .assign te_val.buffer = ( v_var.Name + "->" ) + te_val.buffer
    ...
    .else
      ...
      .assign te_val.buffer = "selected->${te_val.buffer}selected"
      .assign te_val.OAL = "SELECTED."
    .end if
  .else
    .select one o_obj related by v_trv->O_TFR[R829]->O_OBJ[R115]
