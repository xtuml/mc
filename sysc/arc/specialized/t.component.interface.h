.//============================================================================
.// $RCSfile: t.component.interface.h,v $
.//
.// Notice:
.// (C) Copyright 1998-2011 Mentor Graphics Corporation
.//     All rights reserved.
.//
.// This document contains confidential and proprietary information and
.// property of Mentor Graphics Corp.  No part of this document may be
.// reproduced without the express written permission of Mentor Graphics Corp.
.//============================================================================
.//
.//
.//
.if ( "" != c_i.Descrip )
/*
${c_i.Descrip}
*/
.end if
// messages for interface reference ${c_i.Name} (provision)
class ${c_i.Name}_provision\
.if ( (te_sys.SystemCPortsType == "TLM") or (te_sys.SystemCPortsType == "sc_interface") )
 : virtual public sc_interface \
.end if
{
  public:
.if ( te_sys.SystemCPortsType == "BitLevelSignals" )
  ${c_i.Name}_provision (const char* name=sc_gen_unique_name(" ${c_i.Name}_provision "))
  {
  }
.end if
${interface_provision_declarations}\
};

// messages for interface reference ${c_i.Name} (requirement)
class ${c_i.Name}_requirement\
.if ( (te_sys.SystemCPortsType == "TLM") or (te_sys.SystemCPortsType == "sc_interface") )
 : virtual public sc_interface \
.end if
{
  public:
.if ( te_sys.SystemCPortsType == "BitLevelSignals" )
  ${c_i.Name}_requirement (const char* name=sc_gen_unique_name(" ${c_i.Name}_requirement "))
  {
  }
.end if
${interface_requirement_declarations}\
};

.if ( te_sys.SystemCPortsType == "BitLevelSignals" )
// message channel for interface ${c_i.Name} binding
class ${c_i.Name}_channel {
  public:
  ${c_i.Name}_channel (const char* name=sc_gen_unique_name(" ${c_i.Name}_channel "))
  {
  }
${interface_channel_declarations}\
};
.end if
.//
