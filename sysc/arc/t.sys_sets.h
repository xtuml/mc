...
extern void ${te_set.factory}( void );
extern void ${te_set.copy}( ${te_set.base_class} *,
                ${te_set.base_class} * const );
extern void ${te_set.clear}( ${te_set.base_class} * );
extern void ${te_set.insert_element}( ${te_set.base_class} *,
                         void * const );
extern ${te_set.element_type} *
 ...
 */
extern void ${te_dlist.remove_node}( ${te_set.base_class} *,
...
${te_slist.remove_node}( ${te_set.base_class} *,
                      const void * const );
extern void ${te_set.remove_element}( ${te_set.base_class} *,
                         const void * const );
extern const void * ${te_set.contains}( const ${te_set.base_class} * const,
                    const void * const );
extern u2_t ${te_set.element_count}( const ${te_set.base_class} * const );
extern bool ${te_set.equality}( ${te_set.base_class} * const,
                    ${te_set.base_class} * const );
...
extern void * ${te_set.iterator_next}( ${te_set.iterator_class_name} * const );
...
extern u2_t ${te_string.strlen}( const c_t * );

/* We could easily replace this function declaration with a macro
   that invoked the compiler (C library) supplied memset.  */
.// #define ${te_string.memset}( dst, val, len ) memset( dst, val, len )
extern void ${te_string.memset}( void * const, const u1_t, u2_t );

/* We could easily replace this function declaration with a macro
   that invoked the compiler (C library) supplied memmove.  */
.// #define ${te_string.memmove}( dst, src, len ) memmove( dst, src, len )
extern void ${te_string.memmove}( void * const, const void * const, u2_t );
extern void ${te_string.strcpy}( c_t *, const c_t * );
extern c_t * ${te_string.stradd}( const c_t *, const c_t * );
/* We could easily replace this function declaration with a macro
   that invoked the compiler (C library) supplied strcmp.  */
.// #define ${te_string.strcmp}( s1, s2 ) strcmp( s1, s2 )
extern c_t ${te_string.strcmp}( const c_t *, const c_t * );
.// not used but good stuff
.if ( te_sys.InstanceLoading )
extern c_t * ${te_string.itoa}( c_t *, s4_t );
extern s4_t ${te_string.atoi}( const c_t * );
.end if
.if ( 0 != te_sys.UnitsToDynamicallyAllocate )
extern void * ${te_dma.allocate}( const u4_t );
.end if
