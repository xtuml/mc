...
.function MC_metamodel_populate
  ...
  .for each c_c in c_cs
    ...
    .end if
    .select any tm_c from instances of TM_C where ( ( selected.Name == te_c.Name ) and ( selected.Package == te_c.Package ) )
    .if ( not_empty tm_c )
      .// Mark the component with an updated name.
      .assign te_c.Name = tm_c.xName
    .end if
  ...
  .for each s_edt in s_edts
    ...
    .assign te_dt.Include_File = te_dt.Owning_Dom_Name + ( "_datatypes." + te_file.hdr_file_ext )
  ...
  .for each tm_precision in tm_precisions
    ...
    .for each te_dt in te_dts
      ...
      .if ( empty s_cdt )
        .select one s_cdt related by s_dt->S_UDT[R17]->S_CDT[R18]
        .if ( empty s_cdt )
  ...
  .for each act_lnk in act_lnks
    .invoke r = FactoryTE_LNK( act_lnk )
  .end for
  ...
  .for each v_var in v_vars
    ...
    .else
      ...
      .//.assign te_var.buffer = "v_" + "$_{v_var.Name}"
  ...
  .for each te_c in te_cs
    ...
    .for each te_sync in te_syncs
      ...
      .invoke i = FactoryTE_ABA( te_parms, te_c.Name, te_sync.GeneratedName, "S_SYNC", te_dt )
    ...
    .for each te_ee in te_ees
      ...
      .for each s_brg in s_brgs
        ...
        .assign te_brg.GeneratedName = ( te_ee.RegisteredName + "_" ) + s_brg.Name
        .select one te_dt related by s_brg->S_DT[R20]->TE_DT[R2021]
        .select many te_parms related by s_brg->S_BPARM[R21]->TE_PARM[R2028]
        .invoke i = FactoryTE_ABA( te_parms, te_brg.EEkeyletters, te_brg.GeneratedName, "S_BRG", te_dt )
        .assign te_aba = i.te_aba
    ...
    .for each te_class in te_classes
      ...
      .for each o_attr in o_attrs
        ...
        .if ( not_empty o_dbattr )
          ...
          .invoke i = FactoryTE_ABA( te_parms, "", te_dbattr.GeneratedName, "O_DBATTR", te_dt )
          .assign te_aba = i.te_aba
      ...
      .for each o_tfr in o_tfrs
        ...
        .invoke i = FactoryTE_ABA( te_parms, te_class.GeneratedName, te_tfr.GeneratedName, "O_TFR", te_dt )
        .assign te_aba = i.te_aba
...
.function FactoryTE_SM
  ...
  .for each sm_state in sm_states
    ...
    .invoke i = FactoryTE_ABA( empty_te_parms, "", te_act.GeneratedName, "SM_ACT", void_te_dt )
    .assign te_aba = i.te_aba
...
.function FactoryTE_MACT
  ...
  .for each te_parm in te_parms
    .if ( "SystemC" == te_target.language )
      ...
      .if ( ( param_te_dt.Core_Typ == 1 ) and ( te_sys.SystemCPortsType == "TLM" ) )
  ...
  .end if
  .invoke i = FactoryTE_ABA( te_parms, te_mact.ComponentName, te_mact.GeneratedName, "TE_MACT", te_dt )
  .assign te_aba = i.te_aba
...
.function FactoryTE_ABA
  ...
  .for each te_parm in te_parms
    ...
    .end if
  .end for
  .invoke parameters = te_parm_RenderParameters( actual_te_parms )
  .assign te_aba.ParameterDeclaration = parameters.declaration
  .assign te_aba.ParameterDefinition = parameters.definition
  .assign te_aba.ParameterStructure = parameters.structure
  .assign te_aba.ParameterInvocation = parameters.invocation
  .assign te_aba.ParameterAssignment = parameters.assignment
  .assign te_aba.ParameterAssignmentBase = parameters.assignment_base
  .assign te_aba.scope = ""
