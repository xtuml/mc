...
${ws}{ ${te_set.iterator_class_name} ${iterator};
${ws}${te_for.class_name} * ${te_for.class_name}${iterator};
${ws}${te_set.iterator_reset}( &${iterator}, ${te_for.set_variable} );
${ws}while ( (${te_for.class_name}${iterator} = (${te_for.class_name} *)${te_set.iterator_next}( &${iterator} )) != 0 ) {
...
.function t_oal_smt_assign
  ...
  .if ( "" != te_assign.array_spec )
    ...
    .if ( 4 == te_assign.Core_Typ )
      .// string
${ws}${te_string.strcpy}( ${te_assign.lval}, ${te_assign.rval} );
    .else
      .if ( 0 == te_assign.rval_dimensions )
${ws}${te_assign.lval} = ${te_assign.rval};
      .else
${ws}${te_string.memmove}( (void * const) &(${te_assign.lval}), (void const * const) &(${te_assign.rval}), sizeof( ${te_assign.rval} ) );
      .end if
    .end if
  .else
${ws}${te_assign.lval} = ${te_assign.rval};
...
.function t_oal_smt_create_instance
  ...
  .end if
${ws}${var_name} = (${class_name} *) ${te_instance.scope}\
...
.function t_oal_smt_create_event
  ...
  .if ( "" != parameters )
    ...
    .end if
${ws}${var_name} = (${te_evt.GeneratedName} *) ${te_eq.new}( (void *) ${recipient_var_name}, &${te_evt.GeneratedName}c );
...
${ws}${parameters}\
  .else
    ...
    .end if
${ws}${var_name} = ${te_eq.new}( (void *) ${recipient_var_name}, &${te_evt.GeneratedName}c );
  .end if
  .if ( "SystemC" == te_target.language )
${ws}${var_name}->sc_e = &(thismodule->sc_${te_evt.GeneratedName});
  .end if
.end function
.//------------------------------------------------
...
${ws}${method.result}( ${one_var_name}, ${oth_var_name} );
  .else
...
${ws}${method.result}( ${oth_var_name}, ${one_var_name} );
...
${ws}${te_select.var_name} = ${type_cast} ${te_set.get_any}( &${extent_info.extent_var_name} );
  .elif ( te_select.multiplicity == "many" )
    .if ( te_select.is_implicit )
      ...
      .assign attr_declaration = "${te_set.base_class} ${te_select.var_name}_space={0}; ${te_set.base_class} * ${te_select.var_name} = &${te_select.var_name}_space; /* ${te_select.var_name} (${o_obj.Key_Lett}) */"
      .assign attr_deallocation = "${te_set.clear}( ${te_select.var_name} );"
    .end if
    .// The copy method will clear out and free up system resources
    .// used within the set.
${ws}${te_set.copy}( ${te_select.var_name}, &${extent_info.extent_var_name} );
  .else
...
${ws}${te_select_where.var_name} = 0;
  ...
  .elif ( "many" == te_select_where.multiplicity )
    .if ( te_select_where.is_implicit )
      ...
      .assign attr_declaration = "${te_set.base_class} ${te_select_where.var_name}_space={0}; ${te_set.base_class} * ${te_select_where.var_name} = &${te_select_where.var_name}_space; /* ${te_select_where.oal_var_name} (${o_obj.Key_Lett}) */"
      ...
      .// if ( in_loop )
${ws}${te_set.clear}( ${te_select_where.var_name} );
      .// end if
    .else
...
${ws}  while ( (${te_select_where.selected_var_name} = (${te_select_where.class_name} *) ${te_set.iterator_next}( &${iterator} )) != 0 ) {
    .if ( not te_select_where.special )
...
${ws}      ${te_set.insert_element}( ${te_select_where.var_name}, ${te_select_where.selected_var_name} );
    .end if
${ws}    }
${ws}  }
${ws}}
...
.function t_oal_smt_event_parameters
  ...
  .if ( 4 == value_type )
    .// string
    .select any te_string from instances of TE_STRING
    .assign attr_result = "${ws}  ${te_string.strcpy}( ${evt_msg_var}->p_$_{parameter}, ${value} );"
  .else
...
${parameters}
 ...
 ${te_eq.base_event_type} * ${te_eq.event_message_variable} = ${te_eq.new}( ${void_cast}${var_name}, &${te_evt.GeneratedName}c );
  .end if
  .if ( "SystemC" == te_target.language )
  ${te_eq.event_message_variable}->sc_e = &(thismodule->sc_${te_evt.GeneratedName});
  .end if
  .//
  .// Send the event.
  .if ( self_directed )
...
${operation_name}(\
  .if ( uses_thismodule )
 thismodule\
    .if ( ("" != parameters) or instance_based )
, \
    .end if
  .end if
  .if ( instance_based )
