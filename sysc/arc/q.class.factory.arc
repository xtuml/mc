...
${te_class.GeneratedName}_instanceloader( ${te_instance.handle} instance, const c_t * avlstring[] )
{
  ...
  ${te_instance.self}->${te_attr.GeneratedName} = ( '0' != *avlstring[ ${attribute_number} ] );
        ...
        .elif ( cdt.Core_Typ == 2 )
          .// integer
  ${te_instance.self}->${te_attr.GeneratedName} = ${te_string.atoi}( avlstring[ ${attribute_number} ] );
          .assign attribute_number = attribute_number + 1
        .elif ( cdt.Core_Typ == 3 )
          .// real
        .elif ( cdt.Core_Typ == 4 )
  ...
  {i_t i; for ( i = 0; i < avlstring[ ${attribute_number} + 1 ] - avlstring[ ${attribute_number} ]; i++ )
    ${te_instance.self}->${te_attr.GeneratedName}[ i ] = avlstring[ ${attribute_number} ][ i ];
          ...
          .else
  ${te_string.strcpy}( ${te_instance.self}->${te_attr.GeneratedName}, avlstring[ ${attribute_number} ] );
          .end if
          .assign attribute_number = attribute_number + 1
        .elif ( cdt.Core_Typ == 5 )
          .// unique_id
  ${te_instance.self}->${te_attr.GeneratedName} = (${te_instance.handle}) ${te_string.atoi}( avlstring[ ${attribute_number} ] );
  ...
  ${te_instance.self}->${te_attr.GeneratedName} = ${te_string.atoi}( avlstring[ ${attribute_number} ] );
          .assign attribute_number = attribute_number + 1
        .else
  ${te_string.memset}( &${te_instance.self}->${te_attr.GeneratedName}, avlstring[ ${attribute_number} ], sizeof( ${te_instance.self}->${te_attr.GeneratedName} ) );
        .end if
