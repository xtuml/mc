...
.function q_smt_sgn
  ...
  .if ( "SystemC" == te_target.language )
    ...
    .end if
  .end if
  .invoke s = t_oal_smt_iop( name, parameters, te_blk.indentation )
  .assign te_smt.buffer = s.body
  .assign te_smt.OAL = " SEND ${te_mact.PortName}::${te_mact.MessageName}(${parameter_OAL})"
.end function
.//
.// --------------------------------------------------------
...
.function q_smt_iop
  ...
  .invoke s = t_oal_smt_iop( name, parameters, te_blk.indentation )
  .assign te_smt.buffer = s.body
  .assign te_smt.OAL = "${te_mact.PortName}::${te_mact.MessageName}(${parameter_OAL})"
.end function
.//
.// --------------------------------------------------------
...
.function q_smt_bridge
  ...
  .if ( ( "SystemC" == te_target.language ) or ( "C++" == te_target.language ) )
    .assign name = ( s_ee.Key_Lett + "::" ) + name
  .end if
  .invoke s = t_oal_smt_bridge( name, parameters, te_blk.indentation )
  .assign te_smt.buffer = s.body
...
.function q_smt_return
  ...
  .end while
  .//
  .invoke s = t_oal_smt_return( value, intCast1, intCast2, deallocation, te_blk.indentation )
  .assign te_smt.buffer = s.body
        ...
        .if ( 0 == te_lnk.Mult )
          .if ( not te_select_related.by_where )
            ...
            .if ( "subsuper" == te_lnk.assoc_type )
              .select any sub_r_rsub related by te_class->O_OBJ[R2019]->R_OIR[R201]->R_RGO[R203]->R_SUB[R205] where ( selected.Obj_ID == te_class.Obj_ID )
              .if ( not_empty sub_r_rsub )
                .assign cast = ( "(" + te_lnk.te_classGeneratedName ) + " *) "
                .assign b = b + "${te_select_related.result_var} = 0;\n"
                .assign b = b + "if ( ${te_class.system_class_number} == ${te_lnk.left}->R${te_lnk.rel_number}_object_id ) "
              .end if
            .end if
  .//  1  |   T   |  T   |     F     |   "one"      |  0:one   |    F     | select one b related by a->B[R1];
