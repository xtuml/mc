 ...
 */
void
${relate_method.result}( ${part_te_class.GeneratedName} * part, ${form_te_class.GeneratedName} * form )
{
  .if ( te_c.DetectEmpty )
  if ( (part == 0) || (form == 0) ) {
...
${set_referentials}\
  ...
  form->${form_data_member.result} = part;
    .else
  ${te_set.insert_element}( &form->${form_data_member.result}, (${te_set.base_class} *) part );
    .end if
  .else
  ...
  part->${part_data_member.result} = form;
    .else
  ${te_set.insert_element}( &part->${part_data_member.result}, (${te_set.base_class} *) form );
    .end if
  .else
 ...
 */
void
${unrelate_method.result}( ${part_te_class.GeneratedName} * part, ${form_te_class.GeneratedName} * form )
{
  .if ( te_c.DetectEmpty )
  if ( (part == 0) || (form == 0) ) {
...
${reset_referentials}\
  ...
  form->${form_data_member.result} = 0;
    .else
  ${te_set.remove_element}( &form->${form_data_member.result}, (${te_set.base_class} *) part );
    .end if
  .else
  /* Note:  ${form_te_class.Key_Lett}->${part_te_class.Key_Lett}[R${te_rel.Numb}] not navigated */
  .end if
  ...
  part->${part_data_member.result} = 0;
    .else
  ${te_set.remove_element}( &part->${part_data_member.result}, (${te_set.base_class} *) form );
    .end if
  .else
  /* Note:  ${part_te_class.Key_Lett}->${form_te_class.Key_Lett}[R${te_rel.Numb}] not navigated */
  .end if
