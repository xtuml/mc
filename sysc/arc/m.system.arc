...
.function EnableTasking
  ...
  .end if
  .if ( ( ( ( flavor == "Nucleus" ) or ( flavor == "POSIX" ) ) or ( ( flavor == "OSX" ) or ( flavor == "Windows" ) ) ) or ( ( "AUTOSAR" == flavor ) or ( "SystemC" == flavor ) ) )
    .assign tm_thread.flavor = flavor
    .if ( "SystemC" == flavor )
      .// Be sure we have at least 1 thread per component for SystemC.
      .select many c_cs from instances of C_C
      .assign component_count = cardinality c_cs
      .if ( number_of_threads < component_count )
        .assign tm_thread.number_of_threads = component_count
      .end if
    .end if
  .else
