...
.if ( te_sys.TotalContainers > 0 )
  ...
  .if ( te_sys.TotalContainers > 1 )
  for ( i = 0; i < ( ${te_set.number_of_containoids} - 1 ); i++ ) {
    node1s[ i ].next = &node1s[ i + 1 ];
    node1s[ i ].object = 0;
  }
  .end if
  node1s[ ${te_set.number_of_containoids} - 1 ].next = 0;
.else
...
.if ( te_sys.TotalContainers > 0 )
void 
${te_set.copy}( ${te_set.base_class} * to_set,
...
.if ( te_sys.TotalContainers > 0 )
void
${te_set.clear}( ${te_set.base_class} * set )
{
...
.if ( not_empty te_cs )
${te_set.element_type} *
${te_set.insert_block}( ${te_set.element_type} * container,
                       const u1_t * instance,
                       const u2_t length,
                       u2_t count )
{
...
.if ( not_empty te_cs )
u2_t 
${te_set.element_count}( const ${te_set.base_class} * const set )
{
  u2_t result = 0;
  const ${te_set.element_type} * node = set->head;
  while ( node != 0 ) {
...
.if ( not_empty te_cs )
bool
${te_set.equality}( ${te_set.base_class} * const left_set,
 ...
 */
void *
${te_set.iterator_next}( ${te_set.iterator_class_name} * const iter )
{
  void * element = 0;
  if ( iter->cursor != 0 ) {
 ...
 */
void
${te_string.memset}( void * const dst, const u1_t val, u2_t len )
{
  u1_t * d = (u1_t *) dst;
  while ( len > 0 ) {
    len--;
 ...
 */
void
${te_string.memmove}( void * const dst, const void * const src, u2_t len )
{
  u1_t * s = (u1_t *) src;
  u1_t * d = (u1_t *) dst;
  while ( len > 0 ) {
 ...
 */
void
${te_string.strcpy}( c_t * dst, const c_t * src )
{
 ...
 */
c_t *
${te_string.stradd}( const c_t * left, const c_t * right )
{
  static c_t sum_area[ ${te_string.max_string_length} ];
  s2_t i = 0;
  c_t * dst = sum_area;
  while ( ( i < ${te_string.max_string_length} - 1 ) && ( *left != '\0' ) ) {
    ++i;
    *dst++ = *left++;
  }
  while ( ( i < ${te_string.max_string_length} - 1 ) && ( *right != '\0' ) ) {
    ++i;
    *dst++ = *right++;
  }
 ...
 */
c_t
${te_string.strcmp}( const c_t *p1, const c_t *p2 )
{
  const c_t *s1 = p1;
  const c_t *s2 = p2;
  c_t c1, c2;
...
c_t *
${te_string.itoa}( c_t * string, s4_t value )
{
  c_t tmp[16];
  c_t * sp, * tp = tmp;
  s4_t i;
