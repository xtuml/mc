 ...
 */
void
${relate_method.result}( ${te_class.GeneratedName} * left, ${te_class.GeneratedName} * right )
{
  .if ( te_c.DetectEmpty )
  if ( (left == 0) || (right == 0) ) {
...
${set_referentials}\
  ...
  left->${part_data_member.result} = right; /* SR L1 */
    .else
  ${te_set.insert_element}( &left->${part_data_member.result}, right );
    .end if
    .if ( form.Mult == 0 )
  right->${form_data_member.result} = left; /* SR L2 */
    .else
  ${te_set.insert_element}( &right->${form_data_member.result}, left );
    .end if
  .else
    .if ( part.Mult == 0 )
  right->${part_data_member.result} = left; /* SR L4 */
    .else
  ${te_set.insert_element}( &right->${part_data_member.result}, left );
    .end if
    .if ( form.Mult == 0 )
  left->${form_data_member.result} = right; /* SR L5 */
    .else
  ${te_set.insert_element}( &left->${form_data_member.result}, right );
    .end if
  .end if
${persist_relate.body}\
}
 ...
 */
void
${unrelate_method.result}( ${te_class.GeneratedName} * left, ${te_class.GeneratedName} * right )
{
    .if ( te_c.DetectEmpty )
  if ( (left == 0) || (right == 0) ) {
...
${reset_referentials}\
  ...
  right->${form_data_member.result} = 0; /* SR U2 */
      .else
  ${te_set.remove_element}( &right->${form_data_member.result}, left ); /* SR U3 */
      .end if
    .else
  ...
  left->${form_data_member.result} = 0; /* SR U5 */
      .else
  ${te_set.remove_element}( &left->${form_data_member.result}, right ); /* SR U6 */
      .end if
    .end if
